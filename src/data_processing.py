...
class DataLoader:
    ...
    def load_data(filepath: str) -> pd.DataFrame:
        ...
        if "TransactionStartTime" in df.columns:
            ...
            logger.info(
                f"Date range: {df['TransactionStartTime'].min()} to {df['TransactionStartTime'].max()}"
            )
...
class RFMCalculator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X_transformed = X_transformed.merge(
            self.rfm_features_, on="CustomerId", how="left"
        )
    ...
    def _calculate_rfm(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            hasattr(df["TransactionStartTime"].dtype, "tz")
            and df["TransactionStartTime"].dt.tz is not None
        ):
...
class TemporalFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X_transformed["transaction_hour"] = X_transformed[
            "TransactionStartTime"
        ].dt.hour
        X_transformed["transaction_day"] = X_transformed["TransactionStartTime"].dt.day
        X_transformed["transaction_month"] = X_transformed[
            "TransactionStartTime"
        ].dt.month
        X_transformed["transaction_year"] = X_transformed[
            "TransactionStartTime"
        ].dt.year
        X_transformed["transaction_dayofweek"] = X_transformed[
            "TransactionStartTime"
        ].dt.dayofweek
        X_transformed["transaction_weekofyear"] = (
            X_transformed["TransactionStartTime"].dt.isocalendar().week
        )

        # Cyclical features (for hour, dayofweek)
        if self.extract_cyclical:
            X_transformed["hour_sin"] = np.sin(
                2 * np.pi * X_transformed["transaction_hour"] / 24
            )
            X_transformed["hour_cos"] = np.cos(
                2 * np.pi * X_transformed["transaction_hour"] / 24
            )
            X_transformed["dayofweek_sin"] = np.sin(
                2 * np.pi * X_transformed["transaction_dayofweek"] / 7
            )
            X_transformed["dayofweek_cos"] = np.cos(
                2 * np.pi * X_transformed["transaction_dayofweek"] / 7
            )

        # Business hours indicator
        X_transformed["is_business_hours"] = (
            X_transformed["transaction_hour"].between(9, 17).astype(int)
        )

        # Weekend indicator
        X_transformed["is_weekend"] = (
            X_transformed["transaction_dayofweek"].isin([5, 6]).astype(int)
        )

        logger.info(
            f"Extracted {len([c for c in X_transformed.columns if 'transaction' in c or 'hour' in c or 'day' in c])} temporal features"
        )
        return X_transformed
...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def _fit_column_encoder(self, X: pd.DataFrame, col: str, y=None):
        ...
        # For one-hot, we just track unique values
            ...
            unique_vals = (
                X[col].value_counts().head(self.top_n_categories).index.tolist()
            )
...
def process_data_for_training(
    data_path: str, config: Optional[Dict] = None
...
if __name__ == "__main__":
    ...
    try:
        ...
        X_processed, y, pipeline = process_data_for_training(
            data_path="../data/raw/data.csv", config=config
        )

        # Save processed data
        save_processed_data(
            X=X_processed, y=y, output_path="../data/processed/training_data.parquet"
        )
