...
class ModelTrainer:
    ...
    def load_and_prepare_data(self):
        ...
        if self.use_customer_level and os.path.exists(
    ...
    def _evaluate_model(self, model, model_name: str) -> dict:
        ...
        logger.info(
            f"  Confusion Matrix: TP={metrics['tp']}, FP={metrics['fp']}, FN={metrics['fn']}, TN={metrics['tn']}"
        )
    ...
    def _save_evaluation_plots(self, model, model_name: str, y_pred, y_pred_proba, cm):
        ...
        if y_pred_proba is not None:
            ...
            roc_path = (
                f'reports/figures/roc_curve_{model_name.lower().replace(" ", "_")}.png'
            )
    ...
    def hyperparameter_tuning(self, model_type: str = "random_forest"):
        ...
        with mlflow.start_run(run_name=f"{model_type}_tuned"):
            ...
            logger.info(
                f"Starting GridSearchCV with {len(param_grids[model_type])} parameter combinations..."
            )
