...
class TargetVariableCreator:
    ...
    def __init__(
        self, n_clusters: int = 3, method: str = "kmeans", random_state: int = 42
    ):
        """
    ...
    def create_target_from_features(
        self,
        ...
        logger.info(
            f"High-risk customers: {target_df['is_high_risk'].sum()} "
    ...
    def _extract_customer_features(
        self, features_df: pd.DataFrame, customer_id_col: str, rfm_features: list
    ) -> pd.DataFrame:
        """
        Extract unique customer RFM features
        """
        # Get unique customers (take first occurrence of each customer)
        customer_features = features_df.drop_duplicates(
            subset=[customer_id_col], keep="first"
        ).copy()

        # Select only RFM features that exist
        available_features = [f for f in rfm_features if f in customer_features.columns]
        missing_features = [
            f for f in rfm_features if f not in customer_features.columns
        ]
        ...
        customer_features = customer_features[columns_to_keep].set_index(
            customer_id_col
        )
    ...
    def _perform_clustering(self, customer_features: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.info(
            f"Performing {self.method} clustering with {self.n_clusters} clusters..."
        )
    ...
    def _calculate_cluster_statistics(self, clustered_df: pd.DataFrame):
        ...
        for _, row in self.cluster_stats_.iterrows():
            logger.info(
                f"  Cluster {row['cluster']}: {row['size']} customers "
    ...
    def _identify_high_risk_cluster(self, clustered_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for cluster_id in cluster_means.index:
            ...
            recency_score = (
                cluster_means.loc[cluster_id, "recency_days"]
                if "recency_days" in cluster_means.columns
                else 0
            )
        ...
        clustered_df["is_high_risk"] = (
            clustered_df["cluster"] == self.high_risk_cluster_
        ).astype(int)

        return clustered_df

    def _create_target_variable(
        self, clustered_df: pd.DataFrame, customer_id_col: str
    ) -> pd.DataFrame:
        """
    ...
    def visualize_clusters(self, clustered_df: pd.DataFrame, save_path: str = None):
        """
    ...
    def generate_cluster_report(
        self, clustered_df: pd.DataFrame, output_path: str = None
    ) -> dict:
        """
    ...
    def _calculate_feature_importance(self, clustered_df: pd.DataFrame) -> dict:
        ...
        features = [
            ...
            if col not in ["cluster", "is_high_risk"]
        ]
        ...
        for feature in features:
            ...
            importance[feature] = float(
                between_variance / (len(clustered_df) * self.n_clusters)
            )
...
def create_and_merge_target(
    features_path: str, output_path: str = None, config: dict = None
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.info(
            f"   Loaded {len(features_df):,} samples with {len(features_df.columns)} features"
        )
        ...
        if config.get("visualize", True):
            ...
            clustered_customers = pd.concat(
                [customer_features, clustered_customers], axis=1
            )

            viz_path = (
                "reports/figures/rfm_clusters.png"
        ...
        if config.get("save_report", True):
            ...
            target_creator.generate_cluster_report(
                clustered_customers, output_path=report_path
            )
        ...
        logger.info(
            f"   Final dataset: {len(final_df):,} samples, {len(final_df.columns)} columns"
        )
        logger.info(
            f"   Target distribution: {final_df['is_high_risk'].sum():,} high-risk "
